...
function formatTable(lines: string[]): string[] {
	...
	// Filter out separator rows from output
	const dataRows = rows.filter((_, index) => !separatorIndices.has(index));

	// Find the header row index (first row before the first separator)
	const firstSeparatorIndex = Math.min(...separatorIndices);
	const headerRowCount = firstSeparatorIndex > 0 ? firstSeparatorIndex : 0;

	const result: string[] = [];

	// Top border: ┌─────┬─────┬─────┐
	result.push(formatBorderRow(colWidths, "top"));

	// Data rows with optional header separator
	dataRows.forEach((row, index) => {
		...
		if (index === headerRowCount - 1 && index < dataRows.length - 1) {
			result.push(formatBorderRow(colWidths, "middle"));
		}
	});

	// Bottom border: └─────┴─────┴─────┘
	result.push(formatBorderRow(colWidths, "bottom"));
...
function formatBorderRow(
	...
	const chars = {
		top: { left: BOX.topLeft, middle: BOX.topTee, right: BOX.topRight },
		middle: { left: BOX.leftTee, middle: BOX.cross, right: BOX.rightTee },
		bottom: {
			left: BOX.bottomLeft,
			middle: BOX.bottomTee,
			right: BOX.bottomRight,
		},
	};

	const { left, middle, right } = chars[position];
	const segments = colWidths.map((width) => BOX.horizontal.repeat(width + 2));
